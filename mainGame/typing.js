...
// Load test duration dynamically (default 30s)
function getTestDuration() {
  const duration = parseInt(localStorage.getItem("testDuration"));
  return isNaN(duration) ? 30 : duration; // fallback to 30s
}
...
let timer = null;
let gameStart = null;
let gameTime = getTestDuration() * 1000; // dynamically set
...
function newGame() {
  ...
  wordsDiv.innerHTML = '';
  for (let i = 0; i < 200; i++) wordsDiv.innerHTML += formatWord(randomWord());

  addClass(document.querySelector('.word'), 'current');
  addClass(document.querySelector('.letter'), 'current');

  document.getElementById('info').innerHTML = getTestDuration();
  timer = null;
  gameStart = null;
  gameTime = getTestDuration() * 1000;
}

function getWpm() {
  const words = [...document.querySelectorAll('.word')];
  const lastTypedWord = document.querySelector('.word.current');
  const lastIndex = words.indexOf(lastTypedWord) + 1;
  const typedWords = words.slice(0, lastIndex);
